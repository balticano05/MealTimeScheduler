...
public class MealProductsTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int column) {
        ...
        return switch (column) {
            ...
            case 3 -> (Double.parseDouble(item.getProduct().getProtein()) * item.getWeight()) / 100;
            case 4 -> (Double.parseDouble(item.getProduct().getFats()) * item.getWeight()) / 100;
            case 5 -> (Double.parseDouble(item.getProduct().getCarbs()) * item.getWeight()) / 100;
            default -> null;
        };
    }
